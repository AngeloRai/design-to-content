...
const setupAnalysisDirectories = () => {
  const dirs = ['analysis', 'reports', 'focused-work'];
  dirs.forEach(dir => {
...
const saveFocusSession = () => {
  if (currentFocus) {
    const filePath = join(__dirname, '..', 'data', 'focused-work', `focus_${Date.now()}.json`);
...
const saveImplementationPlan = (plan) => {
  const filePath = join(__dirname, '..', 'data', 'reports', `plan_${plan.id}.json`);
