...
import { getRegistryStats, regenerateDependencyRegistry } from '../utils/dependency-registry.js';
...
Respond with JSON only:
{
  ...
  try {
    ...
    console.log(`📊 Strategy: ${decision.strategy} (Complexity: ${decision.complexity_score}/10)`);
    console.log(`💭 Reasoning: ${decision.reasoning}`);

    return decision;
  } catch (error) {
    console.warn('Could not parse AI routing decision, defaulting to ATOM_GENERATION');
...
export const aiAnalyzeComplexity = async (componentSpec) => {
  console.log(`🧠 Analyzing complexity of component: ${componentSpec.name || 'Unknown'}`);
...
Respond with JSON only:
{
  ...
  try {
    ...
    console.log(`  📋 Classification: ${analysis.classification} (Score: ${analysis.complexity_score})`);
    return analysis;
  } catch (error) {
    console.warn('Could not parse complexity analysis, defaulting to atom:', error.message);
    return {
      classification: 'atom',
      complexity_score: 3,
      required_atoms: [],
 ...
 */
export const aiDecideGenerationPath = async (requirements, libraryContext = null) => {
...
Respond with JSON only:
{
  ...
  try {
    ...
    console.log(`🛤️  Generation Path: ${path.path} (Risk: ${path.risk_assessment})`);
    return path;
  } catch (error) {
    console.warn('Could not parse generation path, using default:', error.message);
    return {
...
export const aiEvaluateLibraryContext = async (libraryDocs) => {
  ...
  if (!libraryDocs || (!libraryDocs.atoms && !libraryDocs.molecules)) {
    console.log('  📄 No library context found, proceeding with fresh generation');
    return {
      has_library: false,
      atom_count: 0,
      molecule_count: 0,
...
Respond with JSON only:
{
  ...
  try {
    ...
    console.log(`  📊 Quality: ${evaluation.quality_assessment} | Reusability: ${evaluation.reusability_score}/10`);
    return {
      has_library: true,
...
export const routeComponentGeneration = async (visualAnalysis, figmaData) => {
  try {
    ...
    console.log(`   📊 Dependency Registry: ${registryStats.totalAtoms} atoms, ${registryStats.totalMolecules} molecules`);
    if (registryStats.totalDependencies > 0) {
      console.log(`   🔗 ${registryStats.totalDependencies} total dependencies tracked`);
    }
    ...
    console.log(`📋 Components mentioned in visual analysis: extracting for tracking...`);
    ...
    switch (routingDecision.strategy) {
      case 'ATOM_GENERATION':
        ...
        const atomResult = await generateAndSaveAtoms(visualAnalysis, figmaData, libraryDocs);
        ...
        if (atomResult.stats) {
          console.log(`   📊 Save Results: ${atomResult.stats.new} new, ${atomResult.stats.updated} updated, ${atomResult.stats.skipped} skipped`);
        }

        // Regenerate library docs after atom generation
        if (generatedComponents.length > 0) {
          console.log('🔄 Regenerating library documentation after atom generation...');
      ...
      case 'MOLECULE_GENERATION':
        ...
        debugLogger('MOLECULE_GENERATION_START', { analysis: visualAnalysis.analysis });

        // Use simple molecule generator
        const moleculeResults = await generateMoleculesFromAnalysis(visualAnalysis.analysis);
        ...
        if (generatedComponents.length > 0) {
          console.log('🔄 Regenerating library documentation after molecule generation...');
          await generateLibraryDocs();
      ...
      case 'MIXED_GENERATION':
        ...
        if (routingDecision.priority_order[0] === 'atom') {
          ...
          debugLogger('MIXED_MOLECULE_START', { analysis: visualAnalysis.analysis });

          // Use simple molecule generator
          const mixedMoleculeResults = await generateMoleculesFromAnalysis(visualAnalysis.analysis);
          ...
          debugLogger('MOLECULE_FIRST_START', { analysis: visualAnalysis.analysis });

          const moleculeFirstResults = await generateMoleculesFromAnalysis(visualAnalysis.analysis);
        ...
        if (generatedComponents.length > 0) {
          console.log('🔄 Regenerating library documentation after component generation...');
          await generateLibraryDocs();
      ...
      default:
        ...
        const defaultResult = await generateAndSaveAtoms(visualAnalysis, figmaData);
    ...
    console.log(`Generated ${generatedComponents.length} components using ${routingDecision.strategy} strategy`);
    ...
    const completenessCheck = await validateGenerationCompleteness(visualAnalysis, generatedComponents);
    debugLogger('COMPLETENESS_CHECK', completenessCheck);
    if (completenessCheck.missingComponents.length > 0) {
      console.warn(`⚠️  Completeness Issue: ${completenessCheck.missingComponents.length} expected components were not generated:`);
      completenessCheck.missingComponents.forEach(missing => {
        console.warn(`   - ${missing}: ${completenessCheck.explanations[missing] || 'No explanation provided'}`);
      });
      ...
      if (completenessCheck.highPriorityMissing.length > 0) {
        ...
        const retryResult = await generateMissingComponents(visualAnalysis, completenessCheck.highPriorityMissing);
        if (retryResult.components.length > 0) {
          generatedComponents.push(...retryResult.components);
          console.log(`✅ Successfully generated ${retryResult.components.length} missing components`);
        }
      }
    }
    ...
    if (generatedComponents.length === 0) {
      ...
      console.warn('   - All identified components were contextual (navigation, layout)');
    ...
    const finalResult = {
      ...
      stats: {
        ...
        molecules: generatedComponents.filter(c => c && c.type === 'molecule').length
      }
    };
    ...
    const fallbackResult = await generateAndSaveAtoms(visualAnalysis, figmaData);
    const fallbackComponents = Array.isArray(fallbackResult.components) ? fallbackResult.components : [];
    ...
    const fallbackReturn = {
      ...
      stats: {
        ...
        molecules: fallbackComponents.filter(c => c && c.type === 'molecule').length
      },
      error: error.message
    };
...
Use "low" priority for decorative or secondary elements.`;
    ...
    console.log(`   🎯 AI identified ${components.length} expected components from visual analysis`);

    return Array.isArray(components) ? components : [];

  } catch (error) {
    console.warn('⚠️  AI component extraction failed, using fallback:', error.message);
 ...
 */
const validateGenerationCompleteness = async (visualAnalysis, generatedComponents) => {
  ...
  try {
    ...
    const expectedComponents = await aiExtractExpectedComponents(visualAnalysis.analysis);
    ...
    for (const expectedComponent of expectedComponents) {
      const wasGenerated = generatedNames.some(name =>
        name.toLowerCase().includes(expectedComponent.name.toLowerCase()) ||
        expectedComponent.keywords.some(keyword => name.toLowerCase().includes(keyword.toLowerCase()))
      );

      if (!wasGenerated) {
        missingComponents.push(expectedComponent.name);
        explanations[expectedComponent.name] = `Expected based on visual analysis: "${expectedComponent.evidence}" but not generated`;
    ...
    console.log(`   📊 Analysis: ${missingComponents.length} missing components identified`);
 ...
 */
const generateMissingComponents = async (visualAnalysis, missingComponentTypes) => {
  console.log(`🎯 Targeting generation for: ${missingComponentTypes.join(', ')}`);
