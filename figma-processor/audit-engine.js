  ...
  try {
    ...
    if (!componentPath.includes('/ui/')) {
      return {
        success: true,
        errors: [],
        warnings: ['Component outside ui folder - skipping TypeScript validation'],
        skipped: true
      };
    }
    ...
    diagnostics.forEach(diagnostic => {
      ...
      const line = diagnostic.getStart() ?
        diagnostic.getSourceFile()?.getLineAndColumnAtPos(diagnostic.getStart() || 0).line : 'unknown';
    ...
    if (fileName && !content.includes(`function ${fileName}`) && !content.includes(`const ${fileName}`)) {
      warnings.push(`Component function name should match filename: ${fileName}`);
    }

    const hasErrors = errors.length > 0;

    if (hasErrors) {
      console.log(`    ❌ TypeScript validation: ${errors.length} errors, ${warnings.length} warnings`);
    } else {
    ...
    try {
      ...
      console.log(`    ${basicErrors.length === 0 ? '✅' : '❌'} Basic UI component validation`);
...
export const analyzeComponentStructure = (componentPath) => {
  try {
    ...
    console.log(`    ✅ Structure analysis complete: ${interfaces.length} interfaces, ${functions.length} functions`);
    ...
    console.error(`    ❌ Component structure analysis failed: ${error.message}`);
    return {
      success: false,
      error: error.message
    };
 ...
 */
export const analyzeComponentWithAI = async (componentPath, structureAnalysis = null) => {
  try {
    ...
    const structure = structureAnalysis && structureAnalysis.success
      ? structureAnalysis
...
Respond with structured JSON:
{
  ...
  "overallAssessment": {
    ...
    try {
      ...
      console.warn(`    ⚠️ Could not parse AI response as JSON, using raw response`);
      analysis = { rawResponse: aiResponse, parseError: parseError.message };
    }

    if (analysis) {
      console.log(`    ✅ AI analysis complete: ${analysis.classification || 'unknown'} (Score: ${analysis.overallAssessment?.score || 'N/A'}/10)`);
    }
 ...
 */
export const detectComponentOverlaps = async (targetComponent, existingComponents = []) => {
  try {
    console.log(`    🔍 Overlap detection: ${targetComponent.name || 'unknown'}`);
    ...
    const componentsToCheck = existingComponents.length > 0
      ? existingComponents
    ...
    for (const existing of componentsToCheck.slice(0, 5)) { // Limit to 5 for cost control
      ...
      if (overlapAnalysis.success && overlapAnalysis.analysis.overlapScore > 3) {
        overlapResults.push({
    ...
    console.log(`    ✅ Overlap detection complete: ${overlapResults.length} overlaps found`);
 ...
 */
const analyzeOverlapWithAI = async (component1, code1, structure1, component2, code2, structure2) => {
  try {
    const openai = getOpenAIClient();
...
export const testDesignProcessorIntegration = async () => {
  ...
  try {
    ...
    const mockComponents = [
      {
        ...
        fullPath: '/Users/angraimo/Work/design-to-content/nextjs-app/ui/elements/Label.tsx'
      }
    ];
    ...
    const auditConfig = {
      ...
      maxCostPerComponent: 0.10
    };

    const result = await auditGeneratedComponents(mockComponents, null, auditConfig);

    console.log('Integration Test Result:', result.overallSuccess ? '✅ PASSED' : '❌ FAILED');
...
export const testOverlapDetection = async () => {
  ...
  const textInputPath = join(__dirname, '..', 'nextjs-app', 'ui', 'elements', 'TextInput.tsx');
  ...
  if (existsSync(textInputPath)) {
    ...
    console.log('Overlap Test result:', result.success ? '✅ PASSED' : '❌ FAILED');

    if (result.success) {
      console.log(`Found ${result.overlaps.length} overlaps:`);
      result.overlaps.forEach(overlap => {
        console.log(`  - ${overlap.component}: ${overlap.overlapScore}/10 (${overlap.recommendation})`);
      });
...
const recordCircuitBreakerFailure = (error) => {
  ...
  if (circuitBreakerState.failures.length >= circuitBreakerState.failureThreshold) {
    circuitBreakerState.state = 'open';
    console.warn(`🚨 Circuit breaker OPENED - ${circuitBreakerState.failures.length} failures in ${circuitBreakerState.timeWindow / 1000}s`);
  }
};
...
const resetCostTracker = (maxCostPerComponent = 1.50, maxTotalCost = 15.00) => {
  costTrackerState = {
    totalCost: 0,
    componentCosts: {},
    maxCostPerComponent,
...
const checkCostLimits = (componentName, estimatedCost) => {
  ...
  if (costTrackerState.totalCost + estimatedCost > costTrackerState.maxTotalCost) {
    return {
      allowed: false,
      reason: `Total cost $${costTrackerState.totalCost + estimatedCost} would exceed limit $${costTrackerState.maxTotalCost}`
    };
  }
...
const addCost = (componentName, actualCost) => {
  ...
  costTrackerState.componentCosts[componentName] = (costTrackerState.componentCosts[componentName] || 0) + actualCost;
};
...
const getCostSummary = () => {
  return {
    ...
    utilizationPercent: (costTrackerState.totalCost / costTrackerState.maxTotalCost * 100).toFixed(1),
...
export const auditComponent = async (componentPath, config = {}) => {
  ...
  try {
    ...
    if (!auditResult.safetyChecks.circuitBreakerStatus.allowed) {
      console.warn(`  🚨 Circuit breaker active: ${auditResult.safetyChecks.circuitBreakerStatus.reason}`);
      auditResult.success = false;
      auditResult.issues.push(auditResult.safetyChecks.circuitBreakerStatus.reason);
      return auditResult;
    }

    // Phase 1: TypeScript validation (no API cost)
    if (auditConfig.enableTypeScriptValidation) {
      auditResult.typeScriptValidation = await validateTypeScript(componentPath);
    ...
    if (auditConfig.enableStructureAnalysis) {
      ...
      if (!auditResult.structureAnalysis.success) {
        auditResult.issues.push(`Structure analysis failed: ${auditResult.structureAnalysis.error}`);
      }
    }
    ...
    if (auditConfig.enableAIAnalysis) {
      ...
      auditResult.safetyChecks.costCheck = checkCostLimits(componentName, estimatedCost);

      if (!auditResult.safetyChecks.costCheck.allowed) {
        console.warn(`  💰 Cost limit check failed: ${auditResult.safetyChecks.costCheck.reason}`);
        auditResult.issues.push(`AI analysis skipped: ${auditResult.safetyChecks.costCheck.reason}`);
      } else {
        try {
          auditResult.aiAnalysis = await analyzeComponentWithAI(componentPath, auditResult.structureAnalysis);
          ...
          if (auditResult.aiAnalysis.success) {
            ...
            if (auditResult.aiAnalysis.analysis?.overallAssessment?.improvements) {
              auditResult.recommendations.push(...auditResult.aiAnalysis.analysis.overallAssessment.improvements);
            }
          } else {
            recordCircuitBreakerFailure(new Error(auditResult.aiAnalysis.error));
            auditResult.issues.push(`AI analysis failed: ${auditResult.aiAnalysis.error}`);
          }
        } catch (aiError) {
    ...
    console.log(`  ${auditResult.success ? '✅' : '❌'} Audit complete: ${auditResult.issues.length} issues found${auditConfig.enableAIAnalysis ? `, $${auditResult.totalCost} cost` : ''}`);
    return auditResult;
 ...
 */
export const auditGeneratedComponents = async (components, figmaData = null, config = {}) => {
  const auditConfig = { ...DEFAULT_AUDIT_CONFIG, ...config };
  ...
  if (auditConfig.uiOnly) {
    ...
    if (componentsToAudit.length < components.length) {
      console.log(`🎯 Filtering to UI components: ${componentsToAudit.length}/${components.length} components`);
    }
  }

  console.log(`🔍 AI AUDIT ENGINE - Auditing ${componentsToAudit.length} UI components`);
  ...
  try {
    ...
    for (const component of componentsToAudit) {
      if (!component || !component.path) {
        console.warn(`  ⚠️ Skipping invalid component: ${JSON.stringify(component)}`);
      ...
      if (!circuitStatus.allowed) {
        console.warn(`🚨 Stopping audit due to circuit breaker: ${circuitStatus.reason}`);
        break;
      }
    }
    ...
    if (auditResults.summary.skippedNonUI > 0) {
      console.log(`  🎯 Non-UI components skipped: ${auditResults.summary.skippedNonUI}`);
    }

    if (auditConfig.enableAIAnalysis) {
      console.log(`  💰 Total Cost: $${auditResults.costSummary.totalCost}`);
      console.log(`  📈 Cost Utilization: ${auditResults.costSummary.utilizationPercent}%`);
    }

    if (auditResults.summary.skippedForCost > 0) {
      console.log(`  💸 Skipped for cost limits: ${auditResults.summary.skippedForCost}`);
    }

    if (auditResults.summary.skippedForFailures > 0) {
      console.log(`  🚨 Skipped for failures: ${auditResults.summary.skippedForFailures}`);
    }
...
export const testAIAnalysis = async () => {
  ...
  const testComponentPath = join(__dirname, '..', 'nextjs-app', 'ui', 'elements', 'Label.tsx');

  if (existsSync(testComponentPath)) {
    console.log('Testing AI analysis with Label.tsx component...');
    const config = { enableAIAnalysis: true, enableTypeScriptValidation: false, enableStructureAnalysis: true };
    ...
    if (result.aiAnalysis && result.aiAnalysis.analysis) {
      console.log('AI Classification:', result.aiAnalysis.analysis.classification);
      console.log('Overall Score:', result.aiAnalysis.analysis.overallAssessment?.score);
...
export const testSafetyMechanisms = async () => {
  ...
  // Reset state
  resetCostTracker(0.10, 0.25); // Low limits for testing
  resetCircuitBreaker();
  ...
  console.log('Cost limit test:', costCheck.allowed ? '❌ FAILED' : '✅ PASSED');

  costCheck = checkCostLimits('TestComponent', 0.05); // Should be allowed
  console.log('Cost allowed test:', costCheck.allowed ? '✅ PASSED' : '❌ FAILED');

  // Test cost tracking
  addCost('TestComponent1', 0.08);
  addCost('TestComponent2', 0.12);

  costCheck = checkCostLimits('TestComponent3', 0.10); // Should exceed total
  console.log('Total cost limit test:', costCheck.allowed ? '❌ FAILED' : '✅ PASSED');

  const summary = getCostSummary();
  console.log('Cost tracking test:', summary.totalCost === 0.20 ? '✅ PASSED' : '❌ FAILED');
  ...
  const circuitStatus = checkCircuitBreaker();
  console.log('Circuit breaker test:', !circuitStatus.allowed ? '✅ PASSED' : '❌ FAILED');
  ...
  return {
    success: true,
    costTracking: summary.totalCost === 0.20,
...
export const testAuditEngine = async () => {
  ...
  const testComponentPath = join(__dirname, '..', 'nextjs-app', 'ui', 'elements', 'Input.tsx');
