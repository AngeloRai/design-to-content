...
if (!FIGMA_TOKEN) {
  console.warn(
    "⚠️  FIGMA_ACCESS_TOKEN not found. Figma API features will be disabled."
  );
}
...
export const parseFigmaUrl = (figmaUrl) => {
  try {
    ...
    const urlMatch = figmaUrl.match(
      /design\/([a-zA-Z0-9]+).*node-id=([0-9%A-F-:]+)/i
    );
    if (!urlMatch) {
...
export const fetchNodeData = async (fileKey, nodeId, depth = 4) => {
  try {
    console.log(
      `📏 Fetching node data (depth: ${depth}) for comprehensive component discovery...`
    );
    ...
    console.log(
      `✅ Node data extracted: ${nestedStats.totalNodes} nodes, ${nestedStats.frames} frames, ${nestedStats.textNodes} text nodes`
    );
...
const extractMeasurements = (node) => {
  ...
  if (node.fills && node.fills.length > 0) {
    measurements.colors = node.fills
      .map((fill) => {
  ...
  if (node.strokes && node.strokes.length > 0) {
    measurements.strokes = node.strokes
      .map((stroke) => {
  ...
  if (node.cornerRadius !== undefined) {
    ...
    measurements.borderRadius = node.rectangleCornerRadii.map((r) =>
      Math.round(r)
    );
  }
 ...
 */
export const aiExploreNodes = async (
  fileKey,
  nodeId,
  visualAnalysis,
  explorationGoal = "icons"
) => {
  try {
    ...
    const explorationPlan = await planExplorationWithAI(
      visualAnalysis,
      nodeStructure,
      explorationGoal
    );
    ...
    console.log(
      `🎯 AI identified ${explorationPlan.targetNodes.length} areas to explore`
    );
    ...
    for (const target of explorationPlan.targetNodes) {
      ...
      if (target.needsDeepDive) {
        ...
        if (detailData.nodes && detailData.nodes[target.nodeId]) {
          ...
          const specificNodes = await extractSpecificNodesWithAI(
            detailedNode,
            visualAnalysis,
            explorationGoal
          );
    ...
    console.log(
      `✅ AI-guided exploration complete: found ${discoveredNodes.length} ${explorationGoal}`
    );
 ...
 */
const planExplorationWithAI = async (
  visualAnalysis,
  nodeStructure,
  explorationGoal
) => {
 ...
 */
const extractSpecificNodesWithAI = async (
  detailedNode,
  visualAnalysis,
  explorationGoal
) => {
...
const simplifyNodeForAI = (node, maxDepth, currentDepth = 0) => {
  ...
  if (node.type === "VECTOR" || node.type === "COMPONENT") {
    if (
      simplified.dimensions &&
      simplified.dimensions.width <= 100 &&
      simplified.dimensions.height <= 100
    ) {
  ...
  if (explorationGoal === "icons") {
    const findIconContainers = (node) => {
      if (node.children && node.children.length > 10) {
        ...
        const smallChildren = node.children.filter(
          (child) =>
            child.dimensions &&
...
const extractSpecificNodesFallback = (detailedNode, explorationGoal) => {
  ...
  const extractNodes = (node) => {
    if (explorationGoal === "icons") {
      if (
        (node.type === "VECTOR" || node.type === "COMPONENT") &&
