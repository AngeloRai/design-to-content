 ...
 */
export const generateAtomComponents = async (
  visualAnalysis,
  figmaData,
  ...
  if (
    existingLibrary &&
    existingLibrary.atoms &&
    Object.keys(existingLibrary.atoms).length > 0
  ) {
...
- If existing components need updates, note them but don't regenerate\n`;
    ...
    console.log(
      `   📋 Found ${existingAtoms.length} existing atoms: ${existingAtoms.join(
        ", "
      )}`
    );
  }
  ...
  const nodeStructure = figmaData
    ? await getNodeStructureForAI(figmaData)
    : null;

  const measurementsText = figmaData?.nodeData?.measurements
    ? `\nEXACT MEASUREMENTS FROM FIGMA:\n${JSON.stringify(
        figmaData.nodeData.measurements,
        null,
        2
      )}\n`
    : "";
...
Note: Icons are processed separately, so focus on interactive components, form controls, and UI elements.`;
  ...
  console.log(
    `✅ Generated ${generatedContent.length} characters of atom component code`
  );
...
export const parseAtomComponents = (content) => {
  ...
  console.log(
    `📦 Parsed ${components.length} atom components from AI response:`
  );
  ...
  if (components.length === 0) {
    ...
    console.warn(
      "   - All components were contextual (navigation, layout, etc.)"
    );
  }
 ...
 */
const aiShouldUpdateExistingComponent = async (
  newComponent,
  existingLibraryDoc,
  existingPath
) => {
...
- This atom is used by ${
          ...
          } molecules: ${dependencyImpact.affectedMolecules.join(", ")}
 ...
 */
export const saveAtomComponents = async (
  components,
  existingLibrary = null
) => {
  const saved = [];
  ...
  console.log(
    `💾 Processing ${components.length} atom components for saving...`
  );
      ...
      if (componentExists && existsInLibrary) {
        ...
        if (shouldUpdate) {
          ...
          if (impact.totalDependents > 0) {
            console.log(
              `    ⚠️  Warning: Updating ${component.name} affects ${impact.totalDependents} molecules`
            );
          }
 ...
 */
export const generateAndSaveAtoms = async (
  visualAnalysis,
  figmaData,
  existingLibrary = null
) => {
  ...
  const atomComponents = await generateAtomComponents(
    visualAnalysis,
    figmaData,
    existingLibrary
  );
