...
const parseMoleculeComponents = (content) => {
  ...
  console.log(
    `📦 Parsed ${components.length} molecule components from AI response:`
  );
  ...
  if (components.length === 0) {
    console.warn(
      "⚠️  No components were parsed from AI response. This may indicate:"
    );
    console.warn("   - AI decided no reusable molecules were needed");
    console.warn("   - Parsing error in component extraction");
    console.warn(
      "   - All components were too complex (organisms vs molecules)"
    );
  }
 ...
 */
const aiShouldUpdateExistingMolecule = async (
  newComponent,
  existingLibraryDoc,
  existingPath
) => {
  try {
    ...
    const dependencyContext =
      dependencyImpact.totalDependents > 0
        ? `\nDEPENDENCY IMPACT:
- This molecule is used by ${
            dependencyImpact.totalDependents
          } organisms: ${dependencyImpact.affectedOrganisms.join(", ")}
...
Respond with JSON only:
{
  ...
  "recommendation": "UPDATE" | "SKIP"
    ...
    try {
      ...
      console.log(
        `       Decision: ${evaluation.recommendation} (Confidence: ${evaluation.confidence}/10)`
      );
      ...
      console.warn(
        `    ⚠️  Could not parse AI evaluation for ${newComponent.name}, defaulting to SKIP:`,
        parseError.message
      );
      return false;
    }
    ...
    console.warn(
      `    ⚠️  AI evaluation failed for ${newComponent.name}:`,
      error.message
    );
    ...
    const sizeDifference =
      Math.abs(existingContent.length - newComponent.code.length) /
...
export const generateMolecule = async (componentName, visualAnalysis) => {
  ...
  try {
    ...
    console.log(
      `📋 Built context for ${
        Object.keys(atomContext).length
      } atoms from library docs`
    );
...
Generate the ${componentName} molecule component using atom composition.`;
    ...
    console.log(
      `✅ Generated ${generatedContent.length} characters of molecule component code`
    );
    ...
    const saveResult = await saveMoleculeComponents(
      components,
      existingLibrary
    );

    // Return the first saved component result or null if nothing was saved
    const savedComponent = saveResult.saved.find(
      (comp) => comp.name === componentName
    );
    const skippedComponent = saveResult.skipped.find(
      (comp) => comp.name === componentName
    );
    ...
    if (savedComponent) {
      ...
      try {
        await updateComponentDoc(
          componentName,
          savedComponent.fullPath,
          "molecule"
        );
        console.log(`📚 Updated library docs for ${componentName}`);
      } catch (error) {
        console.warn(
          `⚠️  Failed to update library docs for ${componentName}:`,
          error.message
        );
      }
      ...
      console.error(
        `❌ No result found for ${componentName} in save operation`
      );
      return null;
    }
  } catch (error) {
    ...
    if (jsonMatch) {
      ...
      if (analysisResult.analysis?.componentGroups) {
        analysisResult.analysis.componentGroups.forEach((group) => {
          console.log(
            `   📦 ${group.groupName}: ${group.decision} (${group.reasoning})`
          );
 ...
 */
export const saveMoleculeComponents = async (
  components,
  ...
  console.log(
    `💾 Processing ${components.length} molecule components for saving...`
  );
      ...
      if (componentExists && existsInLibrary) {
        ...
        if (shouldUpdate) {
          ...
          if (impact.totalDependents > 0) {
            console.log(
              `    ⚠️  Warning: Updating ${component.name} affects ${impact.totalDependents} organisms`
            );
          }
          ...
          console.log(
            `  ⏭️  ${component.name} → ${relativePath} (skipped, already exists)`
          );
          skipped.push({
            name: component.name,
            type: component.type,
  ...
  console.log(
    `📊 Save Results: ${saved.length} new, ${updated.length} updated, ${skipped.length} skipped`
  );
...
export const generateMoleculesFromAnalysis = async (visualAnalysis) => {
  ...
  console.log(
    `🎯 AI identified ${componentNames.length} potential molecules:`,
    componentNames
  );
