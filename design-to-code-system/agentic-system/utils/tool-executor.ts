 ...
 * Extracted from core/agent.ts for use in LangGraph workflow
 */
 ...
 */
interface FindSimilarComponentsParams {
  query: string;
  limit?: number;
}
...
interface WriteComponentParams {
  name: string;
  type: string;
  code: string;
}

interface ValidateTypeScriptParams {
  file_path: string;
}

interface CheckCodeQualityParams {
  file_path: string;
}
 ...
 * Tool return types
 */
interface FindSimilarComponentsResult {
  query: string;
  results: Array<{
    name: string;
    type: string;
...
interface WriteComponentResult {
  name: string;
  type: string;
  path?: string;
  error?: string;
...
interface ValidateTypeScriptResult {
  path: string;
  valid: boolean;
  output?: string;
  errors?: string;
...
interface CheckCodeQualityResult {
  path: string;
  valid: boolean;
  errorCount: number;
  warningCount: number;
...
export const createToolExecutor = (
  ...
  const tools = {
    async find_similar_components({
      query,
      limit = 3
    }: FindSimilarComponentsParams): Promise<FindSimilarComponentsResult> {
      if (!vectorSearch) {
        return {
          query,
          results: [],
    ...
    async read_file({ file_path }: ReadFileParams): Promise<ReadFileResult> {
      try {
        ...
        return {
          path: file_path,
          resolved_path: resolvedPath,
          error: `Failed to read file at ${resolvedPath}: ${(error as Error).message}`,
          success: false,
    ...
    async write_component({
      name,
      type,
      code
    }: WriteComponentParams): Promise<WriteComponentResult> {
      try {
    ...
    async validate_typescript({
      file_path
    }: ValidateTypeScriptParams): Promise<ValidateTypeScriptResult> {
      const projectRoot = path.resolve(outputDir, '..');
    ...
    async check_code_quality({
      file_path
    }: CheckCodeQualityParams): Promise<CheckCodeQualityResult> {
      const projectRoot = path.resolve(outputDir, '..');
    ...
    async search_help({
      query,
      context = {}
    }: SearchHelpParams): Promise<SearchHelpResult> {
      try {
        const { searchForHelp } = await import('../tools/search-help.js');
        const result = await searchForHelp(query, context) as { success: boolean; content?: string; error?: string };
        ...
        console.error('      ❌ Search help failed:', (error as Error).message);
        return {
          success: false,
          error: (error as Error).message,
          query,
    ...
    async get_registry(): Promise<GetRegistryResult> {
      ...
      for (const type of ['elements', 'components', 'modules', 'icons'] as const) {
      ...
      const response: GetRegistryResult = {
        summary: {
  ...
  return {
    execute: async (toolName: string, toolInput: Record<string, unknown>): Promise<unknown> => {
      const toolFn = tools[toolName as keyof typeof tools];
      if (typeof toolFn === 'function') {
        return await toolFn(toolInput as never);
      }
