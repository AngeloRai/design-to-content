...
Start by reading the file, then apply fixes systematically.`;
  ...
  const attemptHistory = []; // Track what we've tried for stuck detection
  ...
  while (continueLoop && iterations < maxIterations) {
    ...
    if (response.content) {
      console.log(
        `      Iteration ${iterations}: ${response.content.substring(
          0,
          100
        )}...`
      );
    ...
    if (response.tool_calls && response.tool_calls.length > 0) {
      for (const toolCall of response.tool_calls) {
        ...
        if (functionName === "write_component" && result.success) {
          ...
          const [tsValidation, eslintValidation] = await Promise.all([
          ...
          if (allValid) {
            ...
            attemptHistory.push({
              ...
              approach: response.content?.substring(0, 100) || "standard fix",
            });
          }
        }
  ...
  return (
    lastValidationResult || { success: false, errors: componentDetails.errors }
  );
}

export async function typescriptFixNode(state) {
