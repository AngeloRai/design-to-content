...
import type { ComponentRegistry, ComponentMetadata, ComponentType } from '../types/component.ts';
...
type ExtractedMetadata = z.infer<typeof ComponentMetadataSchema>;

/**
 * Create empty registry structure
 */
export const createEmptyRegistry = (): ComponentRegistry => ({
...
export const findByName = (
  registry: ComponentRegistry,
  name: string
): (ComponentMetadata & { type: ComponentType }) | null => {
  const types: ComponentType[] = ['elements', 'components', 'modules', 'icons'];
...
export const getAllComponents = (
  registry: ComponentRegistry
): Array<ComponentMetadata & { type: ComponentType }> => {
  const all: Array<ComponentMetadata & { type: ComponentType }> = [];
  const types: ComponentType[] = ['elements', 'components', 'modules', 'icons'];
 ...
 */
export const extractComponentMetadata = async (filePath: string): Promise<ExtractedMetadata> => {
  try {
...
- purpose: button, input, container, icon, navigation, display, form, etc`;
    ...
    const metadata = await model.invoke(prompt) as ExtractedMetadata;
    return metadata;
  } catch (error) {
...
export const scanDirectory = async (
  ...
  try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            if (await fs.pathExists(componentFile)) {
              ...
              // Transform props from string[] to ComponentMetadata props format
              const transformedProps = metadata.props.map(propName => ({
                name: propName,
                type: 'unknown',
                required: false,
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error(`Error scanning directory ${directory}:`, errorMessage);
  }
 ...
 */
export const buildRegistry = async (generatedDir: string | null = null): Promise<ComponentRegistry> => {
  ...
  for (const comp of components) {
    registry = addComponent(registry, comp.type as ComponentType, comp);
  ...
  // Update total count
  registry.totalCount = getAllComponents(registry).length;
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  buildRegistry().then(registry => {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error('Failed to build registry:', errorMessage);
    process.exit(1);
  });
}
